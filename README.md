...
8. Release 0.1.7 = multithreading the teclust module.  config.h now generated by build system.
9. Release 0.1.8 = Fix issue #10, which affected "pecnv cnvclust".  The reading of the binary output from "pecnv process" was not portable, causing empty output from "pecnv cnvclust" on some systems".
...
Usage: tclust -h to see help:
  ...
  --threads arg (=1)           Number of threads to use.  (Requires that the bam file have an index!)
